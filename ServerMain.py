...
f.close()
db = pymysql.connect(host=config[0].strip(), port=int(config[1].strip()), user=config[2].strip(),
                     passwd=config[3].strip(), db=config[4].strip(), autocommit=True)
cur = db.cursor()
...
def user_register(username, password, nickname):
    ...
    if cur.rowcount:
        ...
        return cur.execute(
            "INSERT INTO user_information values('" + username + "','" + password + "','" + nickname + "',0);")
